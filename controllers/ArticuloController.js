...
module.exports={
    ...
    update : async (req,res,next)=>{
        try {
            const re = await Articulo.update({categoriaId:req.body.categoriaId, codigo: req.body.codigo, nombre: req.body.nombre, description:req.body.description},{where:{id: req.body.id}})
            res.status(200).json(re);
        } catch (error) {
            res.status(500).json({'error':'oops paso algo'})
            next(error)
